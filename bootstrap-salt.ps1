# Forked from https://github.com/saltstack/salt-bootstrap
# Add proxy support 
...
function Invoke-RequestWithProxy {
    # Function to make a web request with optional proxy settings.
    # Currently, only the parameters shown below are supported and utilized
    # within this script.
...
foreach ($url in $urls) {
    try {
        ...
        $response = Invoke-RequestWithProxy -Uri "$url" `
                    -DisableKeepAlive `
                    -UseBasicParsing `
                    -Method Head `
                    -Proxy $Proxy
...
#Invoke-WebRequest -Uri $saltFileUrl -Proxy $Proxy -ProxyUseDefaultCredentials  -OutFile $localFile
