...
def example_function():
    ...
    # Example usage: getting a specific config value
    s3_key = config.get("s3.key", "default_key_value")
    s3_secret = config.get("s3.secret", "default_secret_value")

    log.info("S3 Key: %s", s3_key)
    log.info("S3 Secret: %s", s3_secret)
...
def query(
    ...
    if local_file and method == "GET":
        if result.status_code < 200 or result.status_code >= 300:
            raise CommandExecutionError(
                "Failed to get file x. {}: {}".format(err_code, err_msg)
            )
